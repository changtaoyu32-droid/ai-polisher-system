...
> **极致极简美学 ✕ 离线随机裂变去重 ✕ 云端大模型流式降AI学术利器**
...
1. **学术 AI 检测率过高**  
   国内外的查重和 AI 检测系统（如**CNKI知网、VIP维普、Wanfang万方、Turnitin、CopyLeaks、GPTZero**）对文章的“AI高频词”（如*基于、通过、优化、实现*）和单一“主谓宾”句式高度敏感。系统通过重塑句式和特定词汇替换，从底层打破机器感。
2. **传统工具千篇一律，改写无变化**  
   普通的离线降重工具死板僵硬，多次点击生成的结果完全相同。本系统全国首创**“本地多同义词随机裂变引擎”**，对同一段原文，每次点击转换都会在同义词池中随机抽取并动态组装，结果呈指数级变化，绝不重样。
3. **API 门槛高与机密隐私泄露**  
   许多在线工具需要将您的论文上传到第三方中转服务器，存在极大的机密泄露风险。本系统的 API 配置信息**100% 存储在用户本地浏览器的 `localStorage` 中**，直接与大模型建立通信。同时内置庞大的**离线学术词库与兜底句式扩展算法**，即使不配置任何 API 也能取得非常优秀的去重效果。
4. **界面杂乱、AI廉价感严重**  
...
- 📉 **多维数据分析仪表盘**：
  ...
  - **字数偏差监控**：字数超限（“输出内容不应多于原文”）时，卡片微抖动并变红报警，保障字数符合硬性要求。
...
本系统使用现代的前端构建工具 **Vite** 驱动，零繁重依赖，极速秒开。
...
### 2. 安装依赖并运行
确保您的电脑上已经安装了 [Node.js](https://nodejs.org/) (建议版本 18.0+)。
```bash
# 安装轻量级 Vite 依赖
npm install
...
启动成功后，控制台会输出本地访问 URL，在浏览器中打开即可：
👉 **`http://localhost:5173/`**
...
如果您希望使用大语言模型（如 DeepSeek、OpenAI、Siliconflow）进行高智能改写，请按以下步骤配置：

1. 点击右上角的 ⚙️ **API 配置** 按钮，滑出侧边栏。
2. 填写服务商的 **API 端点** 与您个人的 **API Key**。
3. 输入想要调用的 **模型名称**（如 `deepseek-chat` 或 `gpt-4o-mini`）。
4. 设置 **重写温度 (Temperature)**，保存即可激活云端流式转换。
...
本项目基于 MIT 协议开源，保留著作权声明即可自由修改与商用。
